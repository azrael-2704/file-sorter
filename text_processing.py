...
def process_single_text_file(args, text_inference, silent=False, log_file=None, suggested_folders=None):
    ...
    with Progress(
        ...
        foldername, filename, description = generate_text_metadata(text, file_path, progress, task_id, text_inference, suggested_folders=suggested_folders)
...
def process_text_files(text_tuples, text_inference, silent=False, log_file=None, suggested_folders=None):
    """Process text files sequentially."""
    results = []
    for args in text_tuples:
        data = process_single_text_file(args, text_inference, silent=silent, log_file=log_file, suggested_folders=suggested_folders)
        results.append(data)
    return results

def generate_text_metadata(input_text, file_path, progress, task_id, text_inference, suggested_folders=None):
...
Filename:"""
    ...
    if suggested_folders:
        folder_suggestion_text = f"\n\nSuggested folders for reuse: {', '.join(sorted(suggested_folders))}\nIf the document fits one of these categories, reuse the folder name. Otherwise, suggest a new category."
